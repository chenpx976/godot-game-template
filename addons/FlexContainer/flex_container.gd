...
#TODO:
#>Add export disable_min_size:bool to avoid loop crashes
...
func _reposition(index, children, content_size):
	if index > 0:
		...
		while true:
			...
			if children[index].get_rect().end.x > rect_size.x+2: #The +2 is a safety net from loop-hell.
...
func _reposition_compact(index, children, content_size):
	...
	if index > 0:
		for i in range(0, (_columns*_rows), 1):
			...
			if children[index].get_rect().end.x > rect_size.x+2:
				flag+=1
